...
func TestGenerate(t *testing.T) {
	...
	expectedAspects := []string{
		...
		"cloudflare-newclient-middleware",
	}
...
func TestGenerateExcludesAllDirectory(t *testing.T) {
	...
	// Count aspects - should be exactly 21 (one per provider, plus openai-v2, 4 for adk, 6 for genkit, 1 for cloudwego/eino, 1 for bedrockruntime, 2 for a2a, and 1 for cloudflare)
	// If it's more, we might be including duplicates from all/
	if len(result.Aspects) != 21 {
		t.Errorf("expected 21 aspects, got %d", len(result.Aspects))
	}
}

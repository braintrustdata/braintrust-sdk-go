...
// Braintrust modification (see VENDOR.md): upstream cases and tests covering
// method dispatch on context values, and mustache lambdas, are removed along
// with the capabilities themselves. Template text arrives from Braintrust and
// must never choose Go code to run.
...
// Braintrust modification (see VENDOR.md): upstream's TestFile, TestFRender and
// TestPartial loaded templates and partials from disk. The filesystem partial
// provider has been removed, so these cover the same ground through
// ParseString and a StaticProvider.
...
// Braintrust modification (see VENDOR.md): partials are rejected outright, so
// upstream's TestPartial is replaced by an assertion that they fail to parse.
func TestPartialsAreRejected(t *testing.T) {
	for _, template := range []string{"{{>partial}}", "{{> /etc/passwd }}", "{{>a/b/c}}"} {
		if _, err := ParseString(template); err == nil {
			t.Errorf("%q parsed; a partial tag must be an error", template)
		}
	}
}
